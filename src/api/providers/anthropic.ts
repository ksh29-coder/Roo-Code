...
export class AnthropicHandler extends BaseProvider implements SingleCompletionHandler {
	...
	async *createMessage(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
		...
		try {
			...
			console.log('\n=== LLM Request Performance Metrics ===')
			console.log(`Model: ${modelId}`)
			console.log(`Time taken: ${timeTaken.toFixed(2)}ms`)
			console.log(`Input tokens: ${totalInputTokens}`)
			console.log(`Output tokens: ${totalOutputTokens}`)
			if (totalCacheWriteTokens > 0) console.log(`Cache write tokens: ${totalCacheWriteTokens}`)
			if (totalCacheReadTokens > 0) console.log(`Cache read tokens: ${totalCacheReadTokens}`)
			console.log('=====================================\n')
		}
	}
